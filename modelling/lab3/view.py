...
class View(QWidget):
    def __init__(self):
        ...
        self.matrix = [
            [
                random.random() / 10 if i != j else 0 for j in range(self.n)
            ]
    ...
    def fill_tables(self):
        ...
        self.stabilization_times = matrix.calc_stabilization_times(self.matrix, self.limit_probabilities)
    ...
    def resize_matrix(self):
        try:
            ...
            new_matrix = [
                [
                    random.random() if i != j else 0 for j in range(self.n)
                ]
                for i in range(self.n)
            ]
