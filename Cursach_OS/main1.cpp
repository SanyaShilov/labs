...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	cout <<Протоколирование событий при управлении процессами;
	getchar();
	cout <<Для работы введите id процессов через пробел;
	getchar();
...
if (argc != 2)
    ...
    if (!status) // если произошла ошибка загрузки 
    {
...
if (k > 0) {
signal(SIGUSR2, f2);
printf("\nПроцесс 2 (%d): получен сигнал от процесса 1 (%d)\n", pid2, pid1);
for (int i=0; i<N; i++) write(f, BufWR, 1);
k--;
sprintf (BufRD, "%d", k);
write(Share[1], BufRD, strlen(BufRD)); 
printf("Процесс 2 (%d): отправляю сигнал процессу 1 (%d)\n", pid2, pid1);
kill(pid1, SIGUSR1);
return; }
    }
...
cout <<Результат сохранен в файл;
